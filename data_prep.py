...
Functions:
- explore_data(analyzer): Run full data exploration workflow.
- process_data(analyzer, method='all'): Run data preprocessing workflow.
- save_analysis_results(sensors, activities, output_file='explorer/analysis_results.txt'): Save analysis results to a text file.
- save_visualizations(activities): Save all visualizations.
- save_enhanced_analysis(sensors, activities, output_dir='explorer'): Save comprehensive analysis results including descriptive stats and visualizations.
...
class SensorDataAnalyzer:
    ...
    def analyze_missing_values(self) -> Dict:
        ...
        for sensor_id, data in self.sensors.items():
            ...
            missing_samples = np.sum(np.all(data == MISSING_VALUE, axis=1))
    ...
    def visualize_distributions(self) -> None:
        ...
        # Sensor distributions by type (histograms)
        ...
        for idx, (sensor_type, info) in enumerate(SENSOR_RANGES.items()):
            all_values = []  # to store valid sensor readings
            for sensor_id in info['sensors']:
                valid_data = self.sensors[sensor_id][self.sensors[sensor_id]
                                                     != MISSING_VALUE]
...
class SensorDataPreprocessor:
    ...
    def preprocess(self, imputation_method: str = 'knn', remove_outliers: bool = True) -> Dict:
        ...
        """
        logger.info(
            f"Preprocessing data using {imputation_method} imputation...")
        ...
        for sensor_id, data in tqdm(self.analyzer.sensors.items(), desc="Processing sensors"):
            ...
            if remove_outliers:
                processed_data = self._remove_outliers(
                    processed_data, sensor_id, imputation_method)
    ...
    def _remove_outliers(self, data: np.ndarray, sensor_id: int, imputation_method: str) -> np.ndarray:
        ...
        data = np.where((data < valid_range[0]) | (
            data > valid_range[1]), np.nan, data)
        ...
        #         data[:, i] = np.where((data[:, i] < lower_bound) | (data[:, i] > upper_bound), np.nan, data[:, i])

        # Impute missing values (outliers replaced with NaN)
        if imputation_method == 'knn':
            data = self._knn_impute(data)
        elif imputation_method == 'interpolation':
            data = self._interpolate(data)
        elif imputation_method == 'spline':
            data = self._spline_impute(data)
        else:
    ...
    def save_preprocessed_data(self, processed_sensors: Dict, method: str) -> None:
        ...
        joblib.dump(processed_sensors, os.path.join(
            output_dir, f'{method}_sensors.pkl'))
        joblib.dump(self.scalers, os.path.join(
            output_dir, f'{method}_scalers.pkl'))
        ...
        with open(summary_file, 'w') as f:
            for sensor_id, data in processed_sensors.items():
                ...
                f.write(f'  Number of time series: {num_time_series}\n')
                f.write(f'  Number of values: {num_values}\n')
                f.write(f'  Mean: {mean_value:.2f}\n')
                f.write(f'  Std: {std_value:.2f}\n')
                f.write('\n')
...
def save_analysis_results(sensors, activities, output_file='explorer/analysis_results.txt'):
    ...
    with open(output_file, 'w') as f:
        ...
        for sensor_type, info in SENSOR_RANGES.items():
            ...
            for sensor_id in info['sensors']:
                ...
                f.write(f"  Sensor {sensor_id}:\n")
                f.write(
                    f"    Range outliers: {len(range_outliers):,} ({range_rate:.2f}%)\n")
                f.write(
                    f"    Statistical outliers: {len(stat_outliers):,} ({stat_rate:.2f}%)\n")
...
def save_enhanced_analysis(sensors, activities, output_dir='explorer'):
    ...
    with open(os.path.join(output_dir, 'analysis_results.txt'), 'w') as f:
        ...
        for sensor_type, info in SENSOR_RANGES.items():
            ...
            for sensor_id in info['sensors']:
                ...
                f.write(
                    f"    Mean: {np.mean(valid_data):.2f} {info['unit']}\n")
                f.write(f"    Std: {np.std(valid_data):.2f} {info['unit']}\n")
                f.write(f"    Min: {np.min(valid_data):.2f} {info['unit']}\n")
                f.write(
                    f"    25%: {np.percentile(valid_data, 25):.2f} {info['unit']}\n")
                f.write(
                    f"    Median: {np.median(valid_data):.2f} {info['unit']}\n")
                f.write(
                    f"    75%: {np.percentile(valid_data, 75):.2f} {info['unit']}\n")
                ...
                f.write(
                    f"    Rate: {(len(range_outliers)/len(valid_data))*100:.2f}%\n")
                ...
                for activity in range(1, 15):
                    ...
                    if np.any(valid_mask):
                        ...
                        f.write(
                            f"      Mean: {np.mean(valid_data):.2f} {info['unit']}\n")
                        f.write(
                            f"      Std: {np.std(valid_data):.2f} {info['unit']}\n")
                        f.write(f"      Valid data: {validity_rate:.1f}%\n")
                        f.write(
                            f"      Samples: {np.sum(valid_mask):,} / {valid_mask.size:,}\n")
                f.write("\n")
    ...
    for sensor_type, info in SENSOR_RANGES.items():
        ...
        plt.savefig(os.path.join(
            output_dir, f'{sensor_type}_distribution.png'))
        plt.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument('mode', choices=['explore', 'process'],
                        ...
                        default='explore')
    parser.add_argument('--method', choices=['all', 'knn', 'interpolation', 'spline'],
